...
def create_genIR_split_plot(csv_file, output_file=None):
    ...
    Modified to address visualization issues with y-axis.
        ...
        if group["name"] == "Fake Image Feedback":
            ...
            if col_idx < len(df.columns):
                ...
                ax1.plot(
                    ...
                    label="Synthetic Image Feedback (Ours)",
                    color=model["color"], 
                    linewidth=2.5,
                    marker=group["marker"],
                    markevery=2,
    ...
    ax2.set_ylim(82, 100)  # Keep zoomed-in view for better visibility of differences
    ax2.set_xlim(0, 10)
    ax2.set_xlabel('Dialog Length', fontsize=14, weight='bold')
    # Remove the redundant y-axis label from the right subplot
    ax2.set_ylabel('')  # Empty label
    ax2.set_title('Comparison of Synthetic Image Feedback Models', fontsize=16, weight='bold')
    ax2.tick_params(axis='both', labelsize=12)
    
    # # Add broken axis indicators to show the right subplot uses a different scale
    # d = .015  # size of the diagonal lines in axes fraction
    # kwargs = dict(transform=ax2.transAxes, color='k', clip_on=False, linewidth=1.5)
    # ax2.plot((-d, +d), (-d, +d), **kwargs)  # bottom-left diagonal
    # ax2.plot((1 - d, 1 + d), (-d, +d), **kwargs)  # bottom-right diagonal
    
    # # Add a note about the different y-axis scales
    # ax2.text(0.5, 0.02, 'Note: Y-axis range is [82-100]', transform=ax2.transAxes, 
    ...
    if no_feedback_handles:
        ...
        legend_labels1.append("Verbal Feedback:")
...
# Example usage:
# create_genIR_split_plot('GenIR results - Figure 1.csv', 'improved_genIR_plot.png')

# Call the function with the filename
if __name__ == "__main__":
    create_genIR_split_plot('paper_writing/GenIR results - Figure 1.csv', 'paper_writing/plot1_3.png')