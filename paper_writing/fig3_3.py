...
def create_dual_dataset_ablation_plot(csv_file, output_file=None):
    ...
    # Define y-limits for each feedback type and dataset
    y_limits = {
        'MSCOCO': {
            'Fake Image Feedback': (65, 100),
            'Prediction Feedback': (65, 100),
            'No Feedback': (65, 100)
        },
        'FFHQ': {
            'Fake Image Feedback': (10, 85),
            'Prediction Feedback': (10, 85),
            'No Feedback': (10, 85)
        }
    }
    ...
    datasets = [
        {'name': 'MSCOCO Hits@10 (in %)', 'data': mscoco_data, 'ylims': y_limits['MSCOCO']},
        {'name': 'FFHQ Hits@10 (in %)', 'data': ffhq_data, 'ylims': y_limits['FFHQ']}
    ]
    ...
    def plot_feedback_type(ax, data, feedback_type, show_ylabel=False, show_xlabel=False, ylim=None):
        ...
        # Remove subplot titles since they're now at the top of columns
        # ax.set_title(f"{feedback_type['name']}", fontsize=14, weight='bold')
    ...
    for row_idx, dataset in enumerate(datasets):
        for col_idx, feedback_type in enumerate(feedback_types):
            ...
            plot_feedback_type(
                ...
                ylim=dataset['ylims'][feedback_type['name']]  # Get specific y-limit for this feedback type
            )
    ...
    # Adjust layout with more space on the left for row titles
    plt.subplots_adjust(left=0.05, right=0.98, top=0.9, bottom=0.1, wspace=0.1, hspace=0.2)
...
if __name__ == "__main__":
    ...
    create_dual_dataset_ablation_plot('paper_writing/GenIR results - Figure 3.csv', 'paper_writing/plot3_3.png')
    print("Visualization complete and saved as 'plot3_3.png'")
